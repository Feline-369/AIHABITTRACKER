...
export const weeklyReport = async (req, res) => {
    try {
        ...
        const userMsg = `Here is my habit data for the past 7 days (${ctx.days[0]}) to (${ctx.days[6]}):\n${ctx.perHabit
        .map(
            (h) =>
                `- ${h.name} (${h.category}, ${h.frequency}): completed ${h.completedDays} of the past 7 days, target ${h.targetDays}/week`)
            .join("\n")}\n\nPlease provide a personalized weekly report now.`;
            ...
            suggestions = [
                {
                    ...
                    category: "Fitness",
                    icon: "🚶‍♂️",
                    reason: "Low-friction way to build consistency early in days."
                },
                {
                    name: "Read 5 pages of a book",
                    description: "Reading can expand your knowledge and reduce stress.",
                    frequency: "daily",
                    category: "Learning",
                    icon: "📚",
                    reason: "Compunds into significant knowledge over weeks and can be very relaxing."
                },
                {
                    name: "2 minutes of deep breathing",
                    description: "Deep breathing can help reduce stress and increase focus.",
                    frequency: "daily",
                    category: "Mindfulness",
                    icon: "🧘‍♀️",
                    reason: "Tiny habit that fits any schedule."
                },
            ];
        }
        await AIInsight.create({
...
export const recoveryPlan = async (req, res) => {
    try {
        ...
        const userMsg =  `Habit: ${habit.name} (${habit.category}, \nDescription: ${habit.description || "none"},\nCurrent streak: ${current} days, \nLongest ever: ${longest} days. The user just broke their streak. Please provide a warm but actionable 3-day recovery plan.`;
        const { content } = await chatCompletion({
...
export const morningMotivation = async (req, res) => {
    try {
        ...
        if (!habits.length) {
            return res.json({ 
                content: "Good morning! Add your habit today and let's get the momentum started.",
                });
        }
        const days = lastNDays(30);
        const logs = await HabitLog.find({
